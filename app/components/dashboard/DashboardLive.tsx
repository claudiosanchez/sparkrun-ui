...
import { DashboardTelemetryProvider, useDashboardStatuses } from "./DashboardTelemetryProvider";
...
export function DashboardLive({
  ...
  return (
    <DashboardTelemetryProvider clusters={clusters} initialStatuses={initialStatuses}>
      <ReactorStateProvider clusters={clusters} initialStatuses={initialStatuses}>
        <div className="flex flex-col gap-6">
          ...
          <DashboardStatusContent clusters={clusters} recipeByCluster={recipeByCluster} />
        </div>
...
function DashboardWorkloadCount({ clusters }: { clusters: ClusterEntry[] }) {
  const statuses = useDashboardStatuses();
  const count = clusters.reduce(
    (total, cluster) => total + (statuses[cluster.name]?.solo_entries.length ?? 0),
    0,
  );
  return (
    <Badge tone="green">
      {count} known managed workload{count === 1 ? "" : "s"}
    </Badge>
  );
}

function DashboardStatusContent({
  clusters,
  recipeByCluster,
}: {
  clusters: ClusterEntry[];
  ...
  const statuses = useDashboardStatuses();
  const savedStatuses = clusters.map((cluster) => ({
    cluster,
    status: statuses[cluster.name] ?? null,
  }));
